...
import { Socials, UserData } from "../utils/interfaces";
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-2">
      ...
      <div className="flex flex-col items-start sm:flex-row gap-2 mb-3">
        ...
        <textarea
          maxLength={120}
          className={`p-2 overflow-y-scroll h-32 w-full rounded-md ${errorFields.includes("bio") ? "border border-red-500" : ""}`}
          value={biography}
          ...
          (platform) => (
            ...
            >
              ...
              <input
                ...
                placeholder={`Enter ${platform} ${platform === "discord" ? "username" : "link"}`}
                className={`p-1 py-0.5 rounded-md w-full ${errorFields.includes(platform) ? "border border-red-500" : ""}`}
              />
            </div>
          ),
        )}
