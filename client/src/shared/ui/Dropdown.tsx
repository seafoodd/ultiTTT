...
const Dropdown: React.FC<DropdownProps> = ({
  ...
  return (
    <div
      className={`${
        className ? className : ""
      } inline-block text-left h-full relative`}
    >
      <div
        onClick={handleTriggerClick}
        className={`h-full px-3 transition-colors ${
          currentMenu === "profile" ? "bg-color-neutral-1000" : ""
        }`}
      >
        {trigger}
      </div>
      ...
      <div
        className={`${
          currentMenu === "profile" ? "scale-y-100" : "scale-y-0"
        } h-fit transform transition-transform origin-top absolute top-14 right-0 w-60 bg-color-neutral-1000/80 box-content shadow-md backdrop-blur-sm z-10`}
      >
        <div>
