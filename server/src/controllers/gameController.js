...
export const handleOverallWin = async (io, game, gameId) => {
  ...
  if (
    ...
    // await saveGameResult(game, overallWinner);
    finishGame(io, game, gameId, overallWinner);
  }
};
...
export const finishGame = async (io, game, gameId, winnerSymbol) => {
...
export const startTimer = (io, game, gameId) => {
  game.timerInterval = setInterval(() => {
    if (game.timers[game.turn] > 0) {
      ...
      finishGame(io, game, gameId, game.turn === "X" ? "O" : "X");
    }
  }, 1000);
};
